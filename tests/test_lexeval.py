...
from dtcalc.lexeval import (next_tok, evaluate, infix_to_postfix,
                            eval_postfix, lexer, sunit_to_td,
                            lexeval, LexError)
...
class TestNextTok:
    @pytest.mark.parametrize("inp,indtfmt,expected", [
        ...
        (" 2021/11/13 ", "%Y/%m/%d",
         (tokens.DTIME(0, 11, datetime.datetime(2021, 11, 13)), 11)),
        (" 2d ad", "%Y/%m/%d",
         (tokens.SUNIT(0, 3, datetime.timedelta(days=2)), 3)),
        (" 32w ad", "%Y/%m/%d",
         (tokens.SUNIT(0, 4, datetime.timedelta(weeks=32)), 4)),
...
class TestEvaluate:
    @pytest.mark.parametrize("op,fst,snd,expected", [
        (tokens.OP(-1, -1, "+"),
         ...
         tokens.DTIME(-1, -1, datetime.datetime(2021, 11, 13, 0, 0))),
...
class TestInfixToPostfix:
    @pytest.mark.parametrize("toks,expected", [
        ...
        ([tokens.DTIME(0, 10, datetime.datetime(2021, 9, 21, 0, 0)),
    ...
    @pytest.mark.parametrize("toks", [
        ...
        ([tokens.SUNIT(0, 2, datetime.timedelta(days=2)),
          tokens.OP(0, 1, '+'), tokens.LPAR(0, 1),
          tokens.SUNIT(0, 2, datetime.timedelta(weeks=3))]),

        # "2d) + 3w "
        ([tokens.SUNIT(0, 2, datetime.timedelta(days=2)),
          tokens.RPAR(0, 2), tokens.OP(0, 1, '+'),
          tokens.SUNIT(0, 2, datetime.timedelta(weeks=3))]),
    ])
...
@pytest.mark.parametrize("toks,expected", [
    ([tokens.DTIME(0, 10, datetime.datetime(2021, 9, 21, 0, 0)),
      ...
      tokens.SUNIT(0, 2, datetime.timedelta(days=21)),
      tokens.OP(0, 1, '-'), tokens.OP(0, 1, '+')],
     tokens.DTIME(-1, -1, datetime.datetime(2021, 9, 2, 0, 0))),
])
...
class TestLexEval:
    @pytest.mark.parametrize("inp,in_dtfmt,out_dtfmt,expected", [
        ("2021/11/09 +  2d", "%Y/%m/%d", "%Y/%m/%d",
         "2021/11/11"),  # datetime.datetime(2021, 11, 11))
        ("2021/11/09 -  2021/11/10", "%Y/%m/%d", "%Y/%m/%d",
         "-1 days"),  # datetime.datetime(2021, 11, 11)),
